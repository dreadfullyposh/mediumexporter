...
function processSection(s, slug, images, options = {}) {
  ...
  if (s.backgroundImage) {
    cleanedImageId = s.backgroundImage.id.replace('*', '')
    const imgwidth = parseInt(s.backgroundImage.originalWidth, 10)
    const imgsrc =
      MEDIUM_IMG_CDN + Math.max(imgwidth * 2, 2000) + '/' + cleanedImageId
    images.push(imgsrc)
    section = '\n![](' + cleanedImageId + ')'
    if (!!options.jekyll) {
      console.log(cleanedImageId)
      section = `\n![](/assets/images/${options.imageFolder + cleanedImageId})`
    }
  }
  return section
}
...
async function processParagraph(p, slug, images, options = {}) {
  ...
  switch (p.type) {
    ...
    case 4: // image & caption
      ...
      cleanedImageId = p.metadata.id.replace('*', '')
      const imgsrc =
        MEDIUM_IMG_CDN + Math.max(imgwidth * 2, 2000) + '/' + cleanedImageId
      ...
      if (!!options.jekyll) {
        console.log(cleanedImageId)
        text = `\n![${p.text}](/assets/images/${slug}/${cleanedImageId})`
      }
      if (p.text) {
        text += '*' + p.text + '*'
      }
